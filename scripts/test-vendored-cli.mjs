...
try {
  ...
  if (metadata.commit) {
    ...
    // Registry-sourced snapshots have no reviewed commit; the checked-out
    // release version is the identity the pin in package.json names.
    const version = JSON.parse(readFileSync(join(source, "package.json"), "utf8")).version;
    assert.equal(version, metadata.version, "CLI test source version must match the bundled snapshot");
  }
  ...
  console.log(`Testing bundled CLI ${metadata.version} (${metadata.commit ?? `${metadata.source} release`}) with ${tests.length} upstream test files`);
